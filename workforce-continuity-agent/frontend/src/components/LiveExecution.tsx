...
const LiveExecution: React.FC<LiveExecutionProps> = ({ activities }) => {
  ...
  const currentProgress = useMemo(() => {
    if (!latestStep || !latestStep.phase) return 0;
    const phaseNum = parseInt(latestStep.phase.split('_')[1]);
    let progress = ((phaseNum - 1) / 6) * 100;
    if (phaseNum === 5 && latestStep.details?.progress_sub) {
      progress += (latestStep.details.progress_sub / 100) * (100 / 6);
    } else {
      progress = (phaseNum / 6) * 100;
    }
  ...
  const getActionLabel = (step: any) => {
    ...
    if (subPhase === 'execution_step') return 'DISK WRITE: IMPLEMENTING';
